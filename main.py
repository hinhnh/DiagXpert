...
def _build_messages_with_history(context: str, user_input: str, hist: list[dict], summary_text: str | None) -> list[dict]:
    ...
    if summary_text:
        messages.append({
            "role": "system",
            "content": "Session summary (for continuity):\n" + summary_text
        })
...
def ask():
    ...
    user_input = (payload.get("question") or "").strip()
...
def get_summary_api():
    session_id = _get_or_create_session_id()
    return jsonify({"session_id": session_id, "summary": get_summary(session_id) or ""})
